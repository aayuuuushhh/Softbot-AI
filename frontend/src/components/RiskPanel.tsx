...
const LEVEL_STYLES: Record<string, { chip: string; rule: string }> = {
  ...
  high: { chip: "bg-orange-50 text-orange-800 ring-orange-200", rule: "bg-orange-700" },
  moderate: { chip: "bg-yellow-50 text-yellow-800 ring-yellow-200", rule: "bg-yellow-600" },
  low: { chip: "bg-green-50 text-green-800 ring-green-200", rule: "bg-green-600" },
};

const FALLBACK = { chip: "bg-slate-100 text-slate-700 ring-slate-200", rule: "bg-slate-400" };
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-diq-line bg-white shadow-sm shadow-slate-900/5">
      <header className="border-b border-diq-line bg-slate-50 px-5 py-3.5">
        <h3 className="text-base font-semibold text-diq-ink">Zone risk assessment</h3>
      ...
      {loading && (
        <p className="px-5 py-8 text-sm text-diq-muted">Assessing zone risk…</p>
      )}
                  ...
                  <dl className="mt-2.5 space-y-1 text-xs leading-5">
                    {risk.population_at_risk && (
                      <div className="flex gap-2">
                        ...
                        <dd className="text-slate-700">{risk.population_at_risk}</dd>
                      </div>
                    )}
                    {risk.access_notes && (
                      <div className="flex gap-2">
